...
public class ChestStealer extends Module {
   ...
   private final BooleanValue silent = ValueBuilder.create(this, "Silent").setDefaultBooleanValue(false).build().getBooleanValue();
   private Screen lastTickScreen;
   private ChestMenu silentMenu;
   ...
   public void onDisable() {
      this.silentMenu = null;
   }

   @EventTarget(1)
   public void onMotion(EventMotion e) {
      if (e.getType() == EventType.PRE) {
         if (this.silent.getCurrentValue() && this.silentMenu != null) {
            this.handleStealing(this.silentMenu);
            return;
         }
   ...
   public void onPacket(EventPacket e) {
      if (!this.silent.getCurrentValue() || e.getType() != EventType.RECEIVE) {
         return;
      }

      if (e.getPacket() instanceof ClientboundOpenScreenPacket packet) {
         ...
         if (!title.equals(chest)
            && !title.equals(largeChest)
            && !title.equals("Chest")
            && !(this.pickEnderChest.getCurrentValue() && title.equals(enderChest))) {
            return;
         }
         ...
         AbstractContainerMenu menu = packet.getType().create(packet.getContainerId(), mc.player.getInventory());
         mc.player.containerMenu = menu;

         if (menu instanceof ChestMenu chestMenu) {
            this.silentMenu = chestMenu;
         }
      }
   }

   private void handleStealing(ChestMenu menu) {
