...
public class Module extends HasValue {
   ...
   protected void initModule() {
      if (this.getClass().isAnnotationPresent(ModuleInfo.class)) {
         ...
         this.setPrettyName();
         Shiori.getInstance().getHasValueManager().registerHasValue(this);
      }
   }
   ...
   public void setEnabled(boolean enabled) {
      try {
         Shiori shiori = Shiori.getInstance();
         if (enabled) {
            this.enabled = true;
            shiori.getEventManager().register(this);
            this.onEnable();
            if (!(this instanceof ClickGUIModule)) {
               HUD module = (HUD) Shiori.getInstance().getModuleManager().getModule(HUD.class);
               ...
               shiori.getNotificationManager().addNotification(notification);
            }
         } else {
            this.enabled = false;
            shiori.getEventManager().unregister(this);
            this.onDisable();
            if (!(this instanceof ClickGUIModule)) {
               HUD module = (HUD) Shiori.getInstance().getModuleManager().getModule(HUD.class);
               if (module.moduleToggleSound.getCurrentValue()) {
